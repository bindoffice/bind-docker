...
与 `docs/` 相同：`make cron` 安装 crontab（默认每天执行一次 `make renew`），`make uncron` 卸载，`sh bin/cert-cron.sh show` 查看。`make renew` 是幂等的，仅在证书剩余时间少于 `CERT_RENEW_DAYS`（默认 30 天）时才真正续期并 reload nginx。详见仓库根目录 [../README.md](../README.md)。
